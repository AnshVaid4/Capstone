...
if monsrcip == "\n" or monsrcport == None or monhostport == None:
    print("[-] All the input fields are necessary")
    quit()
...
def process_packet(packet):
    ...
    if packet.haslayer(IP) and packet[IP] != None:  #IP is scapy packet obj #name 'ip' is not defined
        ...
        if monsrcip != "N" and ((IPv4Address(srcip) in ipobj) or (IPv4Address(dstip) in ipobj)):
